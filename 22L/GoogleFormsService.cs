...
namespace SeatManagerApp
{
    ...
    public class GoogleFormsService
    {
        ...
        public async Task<FormSyncResult> FetchAsync(Func<string, bool> isAlreadyKnown, CancellationToken ct = default)
        {
            ...
            // 기자재 전용 응답 시트를 따로 지정했으면 이어서 읽는다.
            // 이 시트의 행은 "원하는 공간" 질문이 없으므로 전부 기자재 신청으로 본다.
            string equipmentId = (EquipmentSpreadsheetId ?? string.Empty).Trim();
            if (equipmentId.Length > 0 &&
                !string.Equals(equipmentId, SpreadsheetId.Trim(), StringComparison.Ordinal))
            {
                var equipmentResult = new FormSyncResult();
                await FetchSheetAsync(equipmentResult, equipmentId, "기자재", isAlreadyKnown, ct);
                MergeEquipmentResult(result, equipmentResult);
            }

            return result;
        }

        /// <summary>기자재 시트 조회 결과를 메인 결과에 합친다. 기자재 시트만 실패한 경우는 경고로 남긴다.</summary>
        private static void MergeEquipmentResult(FormSyncResult target, FormSyncResult equipment)
        {
            if (!equipment.IsSuccess)
            {
                target.Warnings.Add($"기자재 시트를 읽지 못했습니다 — {equipment.Error}");
                return;
            }

            target.NewRequests.AddRange(equipment.NewRequests);
            target.TotalRows += equipment.TotalRows;
            target.DuplicateCount += equipment.DuplicateCount;

            foreach (string missing in equipment.MissingColumns)
                target.MissingColumns.Add($"(기자재 시트) {missing}");
        }
