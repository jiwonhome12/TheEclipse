...
namespace SeatManagerApp
{
    ...
    public class RentalItem
    {
        ...
        /// <summary>본체(VR/Quest 포함)인지. 본체는 개체를 구분하지 않고 대수만 세므로 번호를 붙이지 않는다.</summary>
        public bool IsMainframe =>
            EquipmentType.Contains("VR") || EquipmentType.Contains("Quest") || EquipmentType.Contains("본체");

        /// <summary>노트북의 개체 번호("No. 17"). 본체이거나 번호를 못 찾으면 빈 문자열.</summary>
        public string UnitNumber => IsMainframe ? string.Empty : ExtractUnitNumber(EquipmentType);

        /// <summary>
        /// 대여품목 표시용. 노트북은 개체 번호를 함께 보여주고, 본체는 번호를 떼고 보여준다.
        /// </summary>
        public string DisplayEquipment
        {
            get
            {
                string baseName = StripUnitNumber(EquipmentType);
                if (IsMainframe) return baseName;

                string no = UnitNumber;
                if (no.Length == 0) return baseName;
                return baseName.Length > 0 ? $"{baseName} ({no})" : no;
            }
        }
